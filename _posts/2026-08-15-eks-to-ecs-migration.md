...
쿠버네티스로 운영하던 서비스를 ECS로 옮기는 작업을 정리했습니다.
"어느 쪽이 더 좋은가"보다는 "이미 EKS에 올라간 것을 어떻게 안 끊기고 옮기는가"에 초점을 맞췄습니다.

## 언제 ECS가 더 맞는가

판단 기준은 대체로 세 가지로 좁혀집니다.

**컨트롤플레인 고정 비용.** EKS는 클러스터가 떠 있기만 해도 시간 단위 고정 요금이 붙습니다.
...
**업그레이드 주기.** 쿠버네티스 마이너 버전은 정해진 기간만 표준 지원을 받고, 그 뒤에는 연장 지원 구간으로 넘어가면서 추가 과금이 붙습니다.
즉 EKS를 쓰는 한 클러스터 업그레이드는 사라지지 않는 반복 업무이고, 애드온(CNI, CoreDNS, kube-proxy) 호환 버전까지 같이 맞춰야 합니다.
ECS에는 이 주기 자체가 없습니다.

**팀 규모 대비 운영 부담.** 인그레스 컨트롤러, cluster-autoscaler 또는 Karpenter, 시크릿 연동, RBAC까지 결국 누군가 계속 봐야 합니다.
...
**로그 경로가 바뀝니다.** Fluent Bit이 파드 로그를 긁어가던 구조에서 `awslogs` 드라이버가 CloudWatch Logs로 직접 보내는 구조가 됩니다. 로그 그룹 이름과 보존 기간을 정하고, 로그를 참조하던 대시보드와 쿼리를 다시 걸어야 합니다. 기존 수집기를 계속 쓰고 싶다면 FireLens로 우회하는 방법도 있습니다.

**메트릭 이름이 바뀝니다.** Prometheus가 kubelet/cAdvisor에서 긁던 컨테이너 메트릭이 사라집니다. Container Insights로 CloudWatch에 모으든지, ADOT 사이드카를 붙여 기존 Prometheus로 계속 보내든지 정해야 합니다. Grafana 대시보드의 쿼리와 알람은 어느 쪽이든 다시 써야 합니다.

**오토스케일링 임계값을 다시 잡아야 합니다.** HPA 설정을 그대로 옮기면 안 됩니다. 태스크당 할당된 vCPU가 파드 리밋과 다르면 같은 부하에서도 CPU 사용률 수치가 다르게 나옵니다.
