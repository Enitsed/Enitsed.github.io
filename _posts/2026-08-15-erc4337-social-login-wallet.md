...
{% comment %} TODO: 페이버릿/페이버스테이션에서 실제로 겪었던 온보딩 이탈 구간이나 지표가 있다면 이 자리에 채워주세요 {% endcomment %}
...
{% comment %} TODO: 페이버릿/페이버스테이션에서 실제로 선택한 방식과 그 판단 근거를 여기에 채워주세요 {% endcomment %}
...
{% comment %} TODO: 실제 운영하며 얻은 교훈이나 모니터링 지표(Grafana/Prometheus로 무엇을 봤는지)를 이 자리에 추가해주세요 {% endcomment %}
