...
자체 구현한 로그인을 Amazon Cognito로 옮기는 작업을 정리했습니다.
신규 서비스에 Cognito를 붙이는 이야기는 문서가 잘 돼 있으니, 여기서는 **이미 회원이 쌓여 있는 서비스를 어떻게 끊기지 않게 옮기는가**만 다룹니다.

## 먼저 부딪히는 제약

이 작업의 모든 설계는 한 문장에서 출발합니다.

> **기존 비밀번호 해시는 Cognito로 가져갈 수 없습니다.**
...
bcrypt든 argon2든 해시는 단방향이라 평문을 복원할 수 없으니 당연한 제약인데, 이걸 놓치고 "회원 테이블을 CSV로 뽑아 임포트하면 되겠다"고 계획을 세우면 **전체 회원에게 비밀번호 재설정을 요구하는 마이그레이션**이 됩니다.
그건 기술 작업이 아니라 이탈률 문제가 됩니다.
...
**커스텀 속성은 나중에 못 바꿉니다.** 이름, 타입, 길이, 변경 가능 여부를 만들 때 정하면 끝이고, 바꾸려면 사용자 풀을 새로 만들어야 합니다.
...
**아이디로 무엇을 쓸지도 되돌릴 수 없습니다.** 이메일을 별칭(alias)으로 쓸지 사용자 이름 자체로 쓸지는 사용자 풀 생성 시점에 결정되고 이후 변경이 안 됩니다.
...
**비밀번호 정책이 레거시보다 엄격하면 문제가 생깁니다.** 예전 정책이 6자였는데 새 정책이 8자 이상 + 특수문자라면, 6자 비밀번호를 쓰던 사용자는 람다 검증은 통과하지만 Cognito가 사용자를 만드는 단계에서 거부합니다.
...
**레거시 회원 DB는 마지막까지 살려둡니다.** Migration 람다가 이걸 읽기 때문입니다.
...
이 작업에서 가장 무서운 건 "절반쯤 옮겨진 상태에서 문제가 발견되는 것"입니다. 그래서 되돌릴 경로를 먼저 만들어둡니다.

**레거시를 읽기 전용으로도 지우지 않습니다.** 위에서 말한 대로 람다가 참조하기도 하고, 문제가 생겼을 때 돌아갈 곳이기도 합니다.

**전환은 클라이언트 배포가 아니라 서버 플래그로 켭니다.** 앱 스토어 심사를 기다려야 하는 구조면 롤백에 며칠이 걸립니다.
로그인 요청을 어느 쪽으로 보낼지 서버가 응답하게 해두면 되돌리는 데 배포 한 번이면 됩니다.

**단계적으로 엽니다.** 내부 계정 → 소수 비율 → 전체 순서로 여는 편이 안전합니다.
Migration 람다는 사용자별로 독립적으로 동작하므로 비율을 나눠 여는 데 특별한 장치가 필요 없습니다. 로그인 요청을 어느 쪽 인증으로 보낼지만 정하면 됩니다.

## 전환 후 확인할 것

**람다 오류율을 봅니다.** Migration 람다가 실패하면 사용자에게는 그냥 "로그인 실패"로 보입니다.
레거시 DB 커넥션 문제로 람다가 터지는 중이면 정상 비밀번호를 넣은 사용자가 계속 튕기는데, 인증 실패 로그만 보면 구분이 안 됩니다. 람다 에러와 인증 실패는 다른 지표로 분리해두세요.

**람다 콜드 스타트가 로그인 지연으로 보입니다.** 이 람다는 로그인 경로에 직접 끼어 있어서, VPC 안에 두고 레거시 DB에 붙는 구조라면 첫 호출이 눈에 띄게 느립니다.
동시성 예약이나 커넥션 재사용을 검토할 지점입니다.

**속성 매핑 누락은 조용히 지나갑니다.** 이름이나 전화번호를 안 넘기면 사용자는 로그인에 성공하고, 나중에 마이페이지에서 빈칸을 봅니다.
이전된 사용자 몇 명을 표본으로 뽑아 속성이 다 찼는지 확인하는 절차를 넣어두면 좋습니다.
