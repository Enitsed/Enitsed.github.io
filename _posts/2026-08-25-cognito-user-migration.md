...
{% comment %} TODO: 실제로 옮긴 회원 규모와, 이전 인증 시스템이 어떤 구조였는지(자체 세션인지 JWT인지, 해시 알고리즘은 무엇이었는지) 적어주세요 {% endcomment %}
...
{% comment %} TODO: 전환 기간을 실제로 얼마나 잡았는지, 미이전 휴면 계정을 어떻게 처리했는지 적어주세요 {% endcomment %}
