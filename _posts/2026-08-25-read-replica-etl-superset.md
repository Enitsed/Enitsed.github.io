...
"결제 추이랑 가입자 수 좀 볼 수 있을까요"에서 시작해 지표 대시보드를 붙이는 작업을 정리했습니다.
BI 도구를 고르는 이야기보다는, **운영 데이터베이스에 부담을 주지 않으면서 지표를 뽑아내는 경로**를 만드는 데 초점을 맞췄습니다.
...
**쿼리 모양을 통제할 수 없습니다.** 대시보드는 보통 `GROUP BY` 한 뒤 기간 필터를 거는 형태인데, 이건 인덱스가 잘 안 듣는 축에 속합니다.
사용자가 기간을 "최근 1년"으로 바꾸는 순간 수백만 행을 훑는 쿼리가 됩니다. 그 쿼리를 누가 언제 돌릴지는 예측할 수 없습니다.

**차단이 아니라 잠식으로 나타납니다.** InnoDB에서 읽기 쿼리가 쓰기를 직접 막지는 않지만, 버퍼 풀은 공유 자원입니다.
집계 쿼리가 콜드한 오래된 페이지를 잔뜩 끌어올리면 운영 쿼리가 쓰던 페이지가 밀려나고, 그때부터 평소에 잘 돌던 쿼리의 지연 시간이 슬금슬금 올라갑니다.
원인을 찾기 어려운 종류의 성능 저하입니다.

**대시보드가 조용히 스키마에 묶입니다.** 대시보드 SQL이 운영 테이블을 직접 참조하면, 그 테이블 구조를 바꿀 때마다 대시보드가 깨집니다.
애플리케이션 코드가 아니라 BI 도구 안에 들어 있어서 배포 시점에 눈에 띄지 않고, 며칠 뒤에 "차트가 비었다"는 이야기로 돌아옵니다.
...
**긴 쿼리가 복제에 밀려 끊깁니다.** 리플리카에서 오래 걸리는 조회를 돌리는 동안 프라이머리에서 같은 행에 대한 변경이 넘어오면, 복제 적용 스레드가 대기하다가 설정된 시간이 지나면 조회 쿼리 쪽이 취소됩니다.
...
여기서 스타 스키마니 팩트/디멘션이니 하는 데이터 웨어하우스 모델링으로 바로 넘어가고 싶어지는데, 대시보드 몇 개 띄우는 단계에서는 대체로 과합니다.
...
**신선도를 대시보드 안에 노출합니다.** 마지막 적재 시각을 보여주는 작은 차트를 하나 두면, 잡이 죽었을 때 "숫자가 이상한데요"가 아니라 "데이터가 안 들어왔네요"로 이야기가 시작됩니다.
...
**잡 실패를 알림으로 받습니다.** 조용히 실패하는 ETL은 틀린 숫자를 보여주는 대시보드보다 나쁩니다. 숫자가 안 바뀌면 아무도 눈치채지 못하기 때문입니다.
스케줄 주기의 두세 배가 지나도록 워터마크가 안 움직이면 알림이 가게 걸어둡니다.

**집계 테이블 크기를 봅니다.** 일별 집계는 카디널리티가 낮아 잘 안 커지지만, 여기에 차원을 하나둘 붙이다 보면 어느새 곱셈이 됩니다.
차원을 추가할 때는 "이 조합이 몇 행이 되는가"를 먼저 계산해 보는 습관이 도움이 됩니다.

**오래된 원본 참조를 정리합니다.** 운영 테이블에서 컬럼이 사라지면 ETL 잡이 깨집니다. 이건 오히려 좋은 신호입니다 — 대시보드가 조용히 비는 대신 잡이 시끄럽게 실패하니까요.
운영 스키마 변경 시 ETL 잡을 같이 고치는 흐름만 만들어두면 됩니다.
