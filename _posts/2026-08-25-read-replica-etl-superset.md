...
{% comment %} TODO: 실제로 어떤 지표부터 요청이 들어왔는지, 그리고 처음에 운영 DB에 직접 붙였다가 겪은 문제가 있었다면 적어주세요 {% endcomment %}
...
{% comment %} TODO: 실제로 어떤 대시보드를 만들었고 사업팀에서 어떻게 쓰고 있는지, 그 과정에서 나온 요구사항 변경이 있었다면 적어주세요 {% endcomment %}
